        ...
        <div class="fitch-proof ${zebraOn ? "zebra-on" : "zebra-off"}">
          ${lines.map((line, index) => {
            ...
            const scopes = baseScope + Array.from({ length: line.depth }, (_, scopeIndex) => {
              ...
              if (level === line.depth && orElimBranchEndLines.has(line.number)) classes.push("scope-branch-end");
            ...
            if (line.reason === "Premise") {
              ...
              if (!nextLine || nextLine.reason !== "Premise") lineClasses.push("premise-divider");
            }
            return `
              <div class="${lineClasses.join(" ")}">
                <div class="fitch-num">${line.number}</div>
                <div class="fitch-main">${scopes}<div class="fitch-content"><div class="fitch-formula-body"><span class="fitch-expr-chip"><span class="fitch-expr">${escapeHtml(formulaToDisplayString(line.formula))}</span></span></div><div class="fitch-reason"><span class="fitch-reason-chip">${escapeHtml(formatFitchReason(line))}</span></div></div></div>
              </div>
            `;
          }).join("")}
        </div>
